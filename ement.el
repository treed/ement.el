...
call `pop-to-buffer'."
  (interactive (list (ement-complete-session) (ement-complete-room)))
                         ...
                         append (cl-loop for room in (ement-session-rooms session)
                                         collect (cons (format "%s (%s)"
                                                               ...
                                                               (or (ement-room-canonical-alias room)
                                                                   (ement-room-id room)))
                                                       (list session room)))))
               (names (mapcar #'car name-to-session-room))
               (selected-name (completing-read "Room: " names nil t)))
    (alist-get selected-name name-to-session-room nil nil #'string=)))
